...
Writes every closed trade to:
  1. data/trade_log.csv  (local backup — ephemeral on Render)
  2. Airtable            (permanent — replaces broken Google Sheets OAuth)
...
# -- Airtable (permanent storage) ---------------------------------------------

def _log_to_airtable(signal_dict: dict):
    """Send to Airtable. Logs warning on failure — never raises, never silent."""
    try:
        from core.airtable import log_trade
        rec_id = log_trade(signal_dict)
        if rec_id:
            logger.info(f"[TradeLog] Airtable OK -> {rec_id}")
        else:
            logger.warning("[TradeLog] Airtable returned None — check API key")
    except Exception as e:
        logger.warning(f"[TradeLog] Airtable failed: {e}")
...
def log_closed_signal(signal: dict):
    ...
    Passes raw signal dict directly to Airtable — no schema translation needed.
    """
    try:
        ...
        pnl_dollar = round(shares * (float(exit_price) - float(entry)), 2) if shares and entry else ""
        ...
        # Pass raw signal dict — airtable.py handles all field mapping
        _log_to_airtable(signal)
...
def log_closed_position(pos: dict):
    """
    Log a closed portfolio position (from portfolio_manager).
    Adapts pos dict to signal-compatible format for Airtable.
    """
    try:
        ...
        # Build signal-compatible dict for Airtable
        _log_to_airtable({
            "ticker":               pos.get("ticker", ""),
            "entry_price":          entry,
            ...
            "entry_session":        pos.get("entry_session", ""),
            "entry_market_context": {"regime": regime},
            "sl":                   pos.get("sl", 0),
            "tp1":                  pos.get("tp1", 0),
